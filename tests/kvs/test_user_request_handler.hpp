...
TEST_F(ServerHandlerTest, UserGetDeltaSetTest) {
  Key key = "key";
  set<string> s;
  s.emplace("value1");
  s.emplace("value2");
  ...
  serializers[LatticeType::SET]->put(key, serialize(SetLattice<string>(s)));
  stored_key_map[key].type_ = LatticeType::SET;
  string serialized_s = serialize(SetLattice<string>(s));

  string get_request = get_key_request_delta(key, ip, serialized_s);
